  ...
  if type(action) == 'function' then
    if is_buffer then
      ...
      if (mode == 'v' or mode == 'x') then
        action = ([[:<C-u>lua mines.mappings._LUA_BUFFER_MAPPINGS[%d]['%s']()<CR>]]):format(bufnr, escaped_key)
      else
        action = ([[<Cmd>lua mines.mappings._LUA_BUFFER_MAPPINGS[%d]['%s']()<CR>]]):format(bufnr, escaped_key)
      end
    else
      M._LUA_MAPPINGS[escaped_key] = action

      if (mode == 'v' or mode == 'x') then
        action = ([[:<C-u>lua mines.mappings._LUA_MAPPINGS['%s']()<CR>]]):format(escaped_key)
      else
        action = ([[<Cmd>lua mines.mappings._LUA_MAPPINGS['%s']()<CR>]]):format(escaped_key)
      end
    end
...
function M.augroup(name, commands)
  ...
  for _, c in ipairs(commands) do
    vim.cmd(string.format('autocmd %s %s %s', table.concat(c.events, ','),
...
function M.create_autocmd(def, name)
  name = name or ('Lua_autocmd_' .. mines.utils.unique_id())

  if type(def[#def]) == 'function' then
    local fn = def[#def]

    def = {unpack(def, 1, #def - 1)}
    table.insert(def, ("lua mines.mappings._LUA_AUGROUP_HOOKS['%s']()"):format(name))
...
function M.create_augroups(definitions)
  if vim.tbl_islist(definitions) then
    ...
    for group_name,defs in pairs(definitions) do
      vim.api.nvim_command('augroup LuaAugroup_' .. group_name)
