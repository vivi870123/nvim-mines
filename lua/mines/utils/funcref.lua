local subscription = require 'mines/utils/subscription'
local unique_id = require 'mines/utils/unique_id'
...
function Funcref:get_vim_ref_string(...)
  return ('function("LuaFunctionRefHandler", [%s])'):format(fn.join({ '"' .. self.name .. '"', ... }, ','))
end
