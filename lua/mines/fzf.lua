...
function Fzf:create(sink, options)
  ...
  instance.subscription = mines.rx.subscription:create()
  instance.sink_ref = sink
  instance.fn_name = 'k' .. mines.utils.unique_id()
  instance.handle_all = options.handle_all
  instance.data = nil

  if type(sink) == 'function' then
    instance.sink_ref = mines.utils.funcref:create(function(ref, results)
...
function Fzf:execute(cmd)
  ...
  if type(self.sink_ref) ~= 'string' then
    mines.command(('let g:%s["sink%s"] = %s'):format(self.fn_name, self.handle_all and '*' or '', self.sink_ref:get_vim_ref_string()))
  else
    mines.command(('let g:%s["sink"] = "%s"'):format(self.fn_name, self.sink_ref))
  end

  mines.command(('call fzf#run(fzf#wrap(g:%s))'):format(self.fn_name))
  mines.command('unlet g:' .. self.fn_name)
end
...
Fzf.float_window = mines.window.float_window
Fzf.create_floating_window = mines.window.create_floating_window
...
function Fzf.grid_to_source(grid, delimiter)
  ...
  return mines.fn.map(grid, function(row)
    return table.concat(row, delimiter)
  end)
end

function Fzf.extract_data_items(results, data)
  return mines.fn.reduce(results, function(res, result)
