...
function M.float_window(on_close)
  if on_close then
    local fn_ref = mines.utils.funcref:create(function(ref)
      on_close()
      ref:unsubscribe()
    end)

    return ("lua mines.window.create_floating_window(%s)"):format(fn_ref:get_lua_ref_string())
  end

  return [[lua mines.window.create_floating_window()]]
end
