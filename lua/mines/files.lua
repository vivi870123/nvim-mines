...
function M.fzf_files(query, fullscreen, history_filename)
  local local_folder = mines.project.create_project_local(vim.fn.expand('%:p:h'))
  local spec = {
    options = {
...
function M.insert_relative_path(from_path)
  local fzf_instance = mines.fzf:create(function(ref, other_path)
    local cwd = vim.fn.getcwd()

    if cwd then
      local lines = mines.fs.exec(
